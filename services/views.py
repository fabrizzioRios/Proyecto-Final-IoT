...
def menu(request):
    if request.method == "POST":
        ...
        if value == '1':
           ...
           program_process = subprocess.run([executable_path, str(len(username)), ip,value], capture_output = True, text = True)
           time.sleep(3)
           context = {
              "value": value,
        ...
        #os.system('./client.out 1')
        #if subscribed:
        #    message = f"You are now subscribed to {topic}"
        #    print(message)
        #subscribe = run(user_data)
        #print(subscribe)
        #else:
           time.sleep(3)
           context = {
        ...
        #    message = f"You were not able to subscribe to {topic}"
           return render(request, 'menu.html', context=context)
    else:
